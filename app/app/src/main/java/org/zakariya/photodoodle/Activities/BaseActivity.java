...
import org.zakariya.photodoodle.BuildConfig;
import org.zakariya.photodoodle.PhotoDoodleApplication;
import org.zakariya.photodoodle.events.ApplicationDidBackgroundEvent;
import org.zakariya.photodoodle.events.ApplicationDidResumeEvent;
	...
	private static class BackgroundWatcher {
		...
		private BackgroundWatcher() {
			...
			action = new Runnable() {
				...
				public void run() {
					...
					PhotoDoodleApplication.getInstance().onApplicationBackgrounded();
		...
		public void onActivityStart() {
			...
			if (count == 1 && didFireBackgroundingEvent) {
				...
				PhotoDoodleApplication.getInstance().onApplicationResumed();
